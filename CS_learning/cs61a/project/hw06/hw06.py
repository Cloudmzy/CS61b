...
class VendingMachine:
    ...
    def vend(self):
        if self.stock<=0:
            ...
            self.stock -= 1
            self.balance = 0 
            return 'Here is your candy and ${0} change.'.format(diff)
        elif self.stock != 0 and self.balance == self.price:
            self.stock -= 1
            self.balance = 0
...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        ...
        return t.branches[0].is_leaf() or is_bst(t.branches[0])
        ...
        l, r = t.branches
        if t.label >= l.label and t.label < r.label:
            return True and is_bst(l) and is_bst(r) and (bst_max(l) <= t.label < bst_min(r))
        else:
            return False
